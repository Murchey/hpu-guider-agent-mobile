<template>
  <div class="map-planning-page" ref="pageRef">
    <div class="map-layout" :style="layoutStyle">
      <div class="sidebar">
...
const sidebarHeightRatio = ref(50) // 默认侧边栏占 50%
...
const layoutStyle = computed(() => {
  if (isMobile.value) {
    return {
      '--sidebar-height': `${sidebarHeightRatio.value}%`,
      '--map-height': `calc(${100 - sidebarHeightRatio.value}% - 16px)` // 减去拖拽条的高度
    }
  }
  return {}
})
...
const resize = (e: TouchEvent) => {
  ...
  // 页面是 column-reverse，sidebar 在下面
  // 手指向上滑（currentY 减小），sidebar 变高；手指向下滑，sidebar 变矮
  const dy = startY - currentY 
  const newHeight = startHeight + dy
  
  const totalHeight = pageRef.value.clientHeight
  let newRatio = (newHeight / totalHeight) * 100
  
  // 限制比例范围 20% - 80%
  newRatio = Math.max(20, Math.min(80, newRatio))
  sidebarHeightRatio.value = newRatio
}
...
const openAmap = () => {
  ...
  // 拼接起点
  url += `from=${start.coordinates.lng},${start.coordinates.lat},${start.name}`
  
  // 拼接终点
  url += `&to=${end.coordinates.lng},${end.coordinates.lat},${end.name}`
  
  // 拼接途经点
  if (waypoints.length > 0) {
    const viaString = waypoints.map(wp => `${wp.coordinates.lng},${wp.coordinates.lat},${wp.name}`).join(';')
    url += `&via=${viaString}`
  }

  // 默认使用驾车模式（car），也可以根据用户第一个路段的模式来判断
  // mode=car|bus|walk|ride
  let mode = 'car'
  if (start.travel_mode === 'walking') mode = 'walk'
  else if (start.travel_mode === 'transit') mode = 'bus'
  else if (start.travel_mode === 'cycling') mode = 'ride'
  
  url += `&mode=${mode}`
  url += `&callnative=1` // 尝试唤起原生App
